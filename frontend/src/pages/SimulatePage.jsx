...
export default function SimulatePage() {
  ...
  const handleDecision = () => {
    ...
    } else if (chosenPath === 'change_idea') {
      navigate('/idea');
    } else if (chosenPath === 'improve') {
      navigate(`/improve/${ideaId}`);
    }
  };
  ...
  const TABS = [
    ...
    { id: 'users', label: '👥 User Perception' },
    { id: 'conflict', label: '⚡ Conflict Engine' },
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-gray-800 px-6 py-4 rounded-xl flex items-center gap-3 shadow-xl border border-gray-700">
            <div className="w-5 h-5 border-2 border-sky-400 border-t-transparent rounded-full animate-spin" />
            <span className="text-white text-sm font-medium">Running simulation...</span>
          </div>
        </div>
      )}
          ...
          <div className="space-y-4">
            {!results.readiness ? (
              ...
              <div className="fade-in space-y-4">
                ...
                <button
                  onClick={() => setResults(prev => ({ ...prev, readiness: null }))}
            ...
            {!results.users ? (
              ...
              <div className="fade-in space-y-4">
                <div className="card">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className={`text-sm font-medium px-2 py-0.5 rounded-full ${
                      results.users.overallSentiment === 'positive' ? 'bg-emerald-900 text-emerald-300' :
                      results.users.overallSentiment === 'negative' ? 'bg-rose-900 text-rose-300' :
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="card">
                    <h3 className="font-semibold text-emerald-400 mb-3">👍 Positive</h3>
                    {(results.users.positiveReactions || []).map((r, i) => (
                      <p key={i} className="text-sm text-gray-300 py-0.5">• {r}</p>
                    ))}
                  </div>
                  <div className="card">
                    <h3 className="font-semibold text-rose-400 mb-3">👎 Concerns</h3>
                    {(results.users.concerns || []).map((c, i) => (
                      <p key={i} className="text-sm text-gray-300 py-0.5">• {c}</p>
                    ))}
                  </div>
                </div>
                <button
                  onClick={() => setResults(prev => ({ ...prev, users: null }))}
                  className="w-full py-2 text-sm text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 rounded-lg transition-colors"
                >
                  🔄 Re-run User Perception
                </button>
              </div>
            )}
          </div>
            ...
            {!results.conflict ? (
              ...
              <div className="fade-in space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="card">
                    <h3 className="font-semibold text-rose-400 mb-3">⚡ Conflict Areas</h3>
                    {(results.conflict.conflict?.conflictAreas || []).map((a, i) => (
                      <p key={i} className="text-sm text-gray-300 py-0.5">• {a}</p>
                    ))}
                  </div>
                  <div className="card">
                    <h3 className="font-semibold text-emerald-400 mb-3">🤝 Alignment Areas</h3>
                    {(results.conflict.conflict?.alignmentAreas || []).map((a, i) => (
                      <p key={i} className="text-sm text-gray-300 py-0.5">• {a}</p>
                    ))}
                  </div>
                </div>
                ...
                <button
                  onClick={() => setResults(prev => ({ ...prev, conflict: null }))}
                  className="w-full py-2 text-sm text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 rounded-lg transition-colors"
                >
        ...
        {activeTab === 'consequences' && (
          <div className="space-y-4">
            <div className="card">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  <button
                    key={path.value}
                    onClick={() => {
                      setChosenPath(path.value);
                      setResults(prev => ({ ...prev, consequences: null }));
                    }}
                    className={`p-3 rounded-xl border text-center transition-all ${
                      chosenPath === path.value
                        ? `border-${path.color}-500 bg-${path.color}-900/20`
              ...
              <button
                onClick={() => runSimulation('consequences')}
                disabled={loading || !chosenPath}
            ...
            {results.consequences && (
              <div className="fade-in space-y-4">
                ...
                <div className={`rounded-2xl p-5 border ${
                  chosenPath === 'continue'    ? 'bg-sky-900/20 border-sky-700' :
                  chosenPath === 'improve'     ? 'bg-amber-900/20 border-amber-700' :
                                                 'bg-violet-900/20 border-violet-700'
                }`}>
                  <p className={`text-sm font-semibold mb-1 ${
                    chosenPath === 'continue'    ? 'text-sky-300' :
                    chosenPath === 'improve'     ? 'text-amber-300' :
                                                   'text-violet-300'
                  }`}>
                    {chosenPath === 'continue'    && '▶ You chose to Continue'}
                    {chosenPath === 'improve'     && '⬆ You chose to Improve'}
                    {chosenPath === 'change_idea' && '🔄 You chose to Change Idea'}
                  </p>
                  <p className="text-xs text-gray-400 mb-4">
                    {chosenPath === 'continue'    && 'Your idea is ready to move to the Pitch Session.'}
                    {chosenPath === 'improve'     && 'AI will suggest specific improvements and ask follow-up questions to sharpen your idea before you pitch.'}
                    {chosenPath === 'change_idea' && "No problem — you'll start fresh with a brand new idea."}
                  </p>
                  <button
                    onClick={handleDecision}
                    className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${
                      chosenPath === 'continue'    ? 'bg-sky-600 hover:bg-sky-500 text-white' :
                      chosenPath === 'improve'     ? 'bg-amber-600 hover:bg-amber-500 text-white' :
                                                     'bg-violet-600 hover:bg-violet-500 text-white'
                    }`}
                  >
                    {chosenPath === 'continue'    && '▶ Proceed to Pitch Session →'}
                    {chosenPath === 'improve'     && '⬆ Start Improvement Flow →'}
                    {chosenPath === 'change_idea' && '🔄 Start a New Idea →'}
                  </button>
                </div>
              </div>
            )}
