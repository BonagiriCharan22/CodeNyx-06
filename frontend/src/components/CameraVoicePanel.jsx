...
export default function CameraVoicePanel({ onTranscript, disabled }) {
  ...
  // ✅ FIX: Assign srcObject AFTER cameraOn flips true and <video> is in the DOM
  useEffect(() => {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
      streamRef.current = stream;
      setCameraOn(true); // <video> will mount on next render, useEffect above handles srcObject
      startListening();
    } catch (err) {
      setCameraError('Camera access denied. Please allow camera access and try again.');
      console.error('Camera error:', err);
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 border-r border-gray-800">
      ...
      {/* ✅ FIX: <video> is always mounted so ref is always available.
           Hidden via CSS when camera is off — never conditionally rendered. */}
      <div className="relative bg-gray-950 aspect-video mx-4 mt-4 rounded-xl overflow-hidden border border-gray-800">
        <video
          ref={videoRef}
          autoPlay
